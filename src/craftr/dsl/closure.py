...
class _ValueRef:
  ...
  def frame_locals(cls, frame: types.FrameType, key: str) -> '_ValueRef':
    ...
    def setter(_v):
      raise RuntimeError('setting outter local variable from Closure is not currently supported')
...
class Closure:
  ...
  * A delegate object
  ...
  def __call__(self, *args, **kwargs) -> t.Any:
    ...
    return self.func(ClosureContextProxy(self), *self.args, *args, **{**self.kwargs, **kwargs})
...
class ClosureContextProxy:
  ...
  def __call__(self) -> t.Any:
    return self.__closure__.delegate

  def __getattr__(self, name: str) -> t.Any:
    return self.__closure__[name]

  def __setattr__(self, name: str, value: t.Any) -> None:
    if name == '__closure__':
      object.__setattr__(self, name, value)
    else:
      self.__closure__[name] = value
